...
public:
  ...
  void  EmitDependenceIntrinsic( const DependenceAttr*  XLXDependence) {
    llvm::Value* addr = nullptr;
    Expr *expr = XLXDependence->getVariable();
    // if "variable=..." option is missing, Parser will generate "IntegerLietrat(1)" 
    ...
    if (expr && !isa<IntegerLiteral>(expr)) {
      if( expr->getType()->isPointerType()){
        //pointer variable
        addr = EmitScalarExpr(expr);
      }
      else {
        //array variable, or struct with only one array field 
        LValue lvalue = EmitLValue(expr);
        addr = lvalue.getPointer();
        assert( addr->getType()->isPointerTy()  &&"unxpected");
        auto baseType = dyn_cast<llvm::PointerType>(addr->getType())->getElementType();
        int GEPIdx = 0;
        while ( baseType->isStructTy()){
          auto ty = dyn_cast<llvm::StructType>(baseType);
          if (ty->getNumElements() != 1) { 
            break;
          }
          baseType = ty->getElementType(0);
          GEPIdx++;
        }
        SmallVector<llvm::Value*, 4> idxs;
        for ( int i = 0; i < GEPIdx; i++ ) { 
        ...
        if (baseType->isPointerTy()) {
          addr = Builder.CreateLoad(Address(addr, lvalue.getAlignment()));
        }
      }
    }
    ...
    auto dep_distance = XLXDependence->getDistance();
